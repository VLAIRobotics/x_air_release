...
def generate_robot_description(context: LaunchContext, description_package, description_file,
                               arm_type, use_fake_hardware, can_interface, arm_prefix, single_side):
    ...
    single_side_str = context.perform_substitution(single_side)
...
def robot_nodes_spawner(context: LaunchContext, description_package, description_file,
                        arm_type, use_fake_hardware, controllers_file, can_interface, arm_prefix, single_side):
...
def generate_launch_description():
    """Generate launch description for XArm unimanual configuration."""
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            "can_interface",
            description="CAN interface to use. No default - the correct "
                        "value depends on this unit's actual physical "
                        "wiring and must be specified explicitly, together "
                        "with single_side.",
        ),
        DeclareLaunchArgument(
            "controllers_file",
            default_value="xarm_v10_controllers.yaml",
        ...
        DeclareLaunchArgument(
            "single_side",
            choices=["right", "left"],
            description="Which physical bimanual-arm mount this single-arm "
                        "unit is actually bolted as - determines the "
                        "gravity model's mount rotation and mirroring. No "
                        "default - depends on actual hardware topology, "
                        "must be specified explicitly together with "
                        "can_interface (not auto-derived or cross-checked).",
        ),
    ]
