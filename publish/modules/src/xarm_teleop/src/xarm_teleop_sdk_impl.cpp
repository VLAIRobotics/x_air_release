...
struct TeleopSession {
    ...
    // 用户指定的归位位置（覆盖 INITIAL_POSITION 常量），空时使用默认值
...
int xarm_teleop_start(xarm_teleop_handle_t h) {
    ...
    try {
        if (sess->mode == SessionMode::kGravityComp) {
            ...
            if (!sess->home_position.empty()) {
                sess->leader_ctrl->SetInitialPosition(sess->home_position);
                sess->follower_ctrl->SetInitialPosition(sess->home_position);
            }
...
int xarm_teleop_set_home_position(xarm_teleop_handle_t h,
    ...
    if (positions == nullptr || count <= 0) {
        set_error("invalid positions argument");
        return XARM_TELEOP_ERR_PARAM;
    }
    sess->home_position.assign(positions, positions + count);
    set_error("");
    return XARM_TELEOP_OK;
}
