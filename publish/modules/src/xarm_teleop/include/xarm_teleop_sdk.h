 ...
 * @brief 设置归位目标位置（初始位置）
 *
 * 覆盖默认的 INITIAL_POSITION，指定遥操作启动前两臂运动到的目标关节角度。
 * 必须在 xarm_teleop_create_* 之后、xarm_teleop_start 之前调用，或者在
 * xarm_teleop_go_home 之前调用以改变下次归位的目标位置。
 *
 * @param h          有效的遥操作会话句柄
 * @param positions  各关节目标位置数组（弧度），长度为 count
 * @param count      关节数量（通常为 7，不含夹爪）
 * @return XARM_TELEOP_OK (0) 表示成功，负值表示错误码
 */
