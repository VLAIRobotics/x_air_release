...
#
# 使用方法:
#   ./start_xarm_teleop.sh [mode] [arm_side] [leader_can] [follower_can] [--home j1,j2,j3,j4,j5,j6,j7]
#
...
# --home      : 初始关节位置（弧度），逗号分隔的7个值，如 0,0,0,0.628,0,0,0（仅 unilateral 模式有效）
#
...
#   ./start_xarm_teleop.sh unilateral right_arm can0 can2 --home 0,0,0,0.628,0,0,0
...
echo "  工作空间    : $WS_DIR"
if [[ -n "$HOME_POSITION" ]]; then
echo "  初始位置    : $HOME_POSITION"
fi
echo "========================================"
...
export LD_LIBRARY_PATH="$SDK_LIB_DIR:/usr/local/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
