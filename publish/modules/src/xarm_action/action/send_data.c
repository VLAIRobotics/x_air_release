...
#define FREQ_5KHZ_NS 200000LL // 5kHz = 200us = 200000ns
...
static void run_send_from_log(void) {
    char choice[8];
    char line[MAX_LOG_LINE];
    long long send_interval_ns = FREQ_5KHZ_NS;
    const int use_high_precision = 1;
    const int use_two_freq = 1;
    ...
    if (choice[0] == '1') {
        ...
        if (use_two_freq) {
            params.use_two_freq = 1;
            params.first_interval_ns = 1000000LL;      // 1kHz = 1000us = 1000000ns
            params.second_interval_ns = 200000LL;      // 5kHz = 200us = 200000ns
            params.switch_frame_count = 6000;
        }
        ...
        printf("[INFO] CAN 口: %s\n", params.can_if);
        if (use_two_freq) {
            printf("[INFO] 发送频率: 前6000帧 1kHz, 之后 5kHz\n");
        } else {
        ...
        if (use_two_freq) {
            params[0].use_two_freq = 1;
            params[0].first_interval_ns = 1000000LL;      // 1kHz = 1000us = 1000000ns
            params[0].second_interval_ns = 200000LL;      // 5kHz = 200us = 200000ns
            params[0].switch_frame_count = 8000;
            
            params[1].use_two_freq = 1;
            params[1].first_interval_ns = 1000000LL;
            params[1].second_interval_ns = 200000LL;
            params[1].switch_frame_count = 8000;
        }
