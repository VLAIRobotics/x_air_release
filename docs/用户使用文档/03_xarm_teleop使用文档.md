...
| 扩展能力 | `xarm_teleop_set_home_position` / `xarm_teleop_set_joint_state_callback` / `xarm_teleop_set_full_state_callback` / `xarm_teleop_go_home` |
...
释放所有资源。若会话仍在运行会自动先调用 `stop`。调用后句柄无效，不可再使用。

---

#### 5.3.9 `xarm_teleop_set_home_position` — 设置归位目标位置

```c
int xarm_teleop_set_home_position(
    xarm_teleop_handle_t h,
    const double*        positions,  // 各关节目标位置（弧度），长度为 count
    int                  count       // 关节数量（通常为 7，不含夹爪）
);
```

覆盖默认初始位置（`{0, 0, 0, π/5, 0, 0, 0}`），指定遥操作启动前两臂运动到的目标关节角度。

**调用时机**：`create_*` 成功之后、`start` 或 `go_home` 之前。可随时更新，下次归位时生效。

```c
// 示例：将第 4 关节目标位置改为 0.5 rad，其余归零
double home[7] = {0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0};
xarm_teleop_set_home_position(h, home, 7);
xarm_teleop_start(h);
```

未调用此函数时，行为与之前完全相同（使用编译期默认值）。

---

#### 5.3.10 `xarm_teleop_set_joint_state_callback` — 注册 Leader 关节状态回调

```c
typedef void (*xarm_teleop_joint_state_cb_t)(
    const float* leader_positions,  // Leader 臂各关节位置（rad），共 arm_dof 个
    int          arm_dof,           // 关节数量（通常为 7）
    float        gripper_position,  // 夹爪位置（rad）
...
每个控制周期（约 2ms）触发一次，传入 Leader 最新关节状态。传入 `NULL` 取消注册。用于 ROS2 集成时发布 `/joint_states` 话题，或用于数据采集/可视化。

---

#### 5.3.11 `xarm_teleop_set_full_state_callback` — 注册双臂完整状态回调

```c
typedef void (*xarm_teleop_full_state_cb_t)(
    const float* leader_arm_positions,    // Leader 臂各关节位置（rad），共 arm_dof 个
    int          arm_dof,                 // 关节数量（通常为 7）
    float        leader_gripper,          // Leader 夹爪位置（rad）
    const float* follower_arm_positions,  // Follower 臂各关节位置（rad），共 arm_dof 个
    float        follower_gripper,        // Follower 夹爪位置（rad）
    void*        user_data
);

int xarm_teleop_set_full_state_callback(
    xarm_teleop_handle_t       h,
...
同时提供 Leader 和 Follower 两臂的关节状态，适用于需要同时发布双臂话题的场景。仅对单边/双边模式有效，重力补偿模式下不触发。

---

#### 5.3.12 `xarm_teleop_go_home` — 执行归位

```c
int xarm_teleop_go_home(xarm_teleop_handle_t h);
```

控制 Leader/Follower 两臂运动到归位目标位置（由 `set_home_position` 设定，或默认值）。内部执行线性插值运动，期间**阻塞**，完成后恢复控制循环。

---

#### 5.3.13 `xarm_teleop_get_last_error` / `xarm_teleop_version`
...
`get_last_error` 返回最近一次 API 调用的错误描述字符串（线程安全，调用者不需释放），用于调试和日志。`version` 返回版本字符串，如 `"1.0.0"`。
...
```text
create_* → [set_home_position] → [set_*_callback] → start
         → [is_running / go_home]
         → stop → destroy
```

`set_home_position` 和 `set_*_callback` 均为可选，不调用时使用默认行为。`go_home` 可在运行中随时触发，会短暂暂停控制循环后自动恢复。
